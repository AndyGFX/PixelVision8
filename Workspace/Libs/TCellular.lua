...
-- TCellular v0.51 [AndyGFX] ------------------------------------------------------

--[[

V0.51
- added DUMP method for export map data to log.file
- fixed preview map position on display
...
copy file TCellular.lua to Lib folder
...
function TCellular:PreviewMap(ox,oy)
	...
	for xm=1,cave._w do
		for ym=1,cave._h do
			DrawPixels({cave:Map(xm,ym)+11},ox+xm,oy+ym,1,1);
		end
	end
end
